...
2. 作者（字符串，非空）；
3. 出版社（字符串，非空）；
4. 出版年份（整数，`0 < year < 2100`）；
5. 库存数量（整数，`>= 0`）。

系统自动生成图书编号（如 `B009`，按现有最大编号递增），馆藏总数与库存相同、价格默认 0，录入成功回显新书信息；任一字段为空或非法时取消录入，不写入数据。

选择 `2` 进入**删除图书信息**：输入要删除图书的编号或书名，系统查找对应节点，找到则从链表中移除并释放内存（提示"图书删除成功"），未找到提示"未找到编号或名称为 xxx 的图书"；输入为空则取消删除。

选择 `3` 进入**图书信息更新**：输入要更新图书的编号或书名，系统查找对应节点，未找到提示"未找到编号或名称为 xxx 的图书"；找到则先显示当前信息，随后逐项提示输入新的书名/作者/出版社/出版年份/库存数量，每项**直接回车即保留原值**；出版年份需满足 `0 < year < 2100`、库存数量需 `>= 0`，非法输入该项保留原值并提示。更新完成回显新信息并提示"图书信息更新成功"。
...
1. `main.c` 新增 `printBookBrief`（仅输出书名/作者/出版社/出版年份/库存数量）与 `showBookList`（空表给出提示，否则列标题 + 逐本列表展示）；
2. 管理员功能菜单新增"4.图书信息显示"。

验证：`gcc -Wall -Wextra` 编译零警告；有数据列表展示 / 空表提示 / 菜单入口正确路径实测通过。

**2026-08-13：开发图书信息更新模块**

1. `main.c` 新增 `updateBookInfo`：按编号或书名定位图书（复用 `findBookById` / `findBookByName`），逐项更新书名/作者/出版社/出版年份/库存数量，回车保留原值，年份与库存做范围校验；
2. 管理员功能菜单新增"3.图书信息更新"。

验证：`gcc -Wall -Wextra` 编译零警告；按编号更新 / 按书名更新 / 未找到 / 空输入取消 / 回车保留原值 / 非法年份与负数库存路径实测通过。

**2026-08-13：开发图书信息删除模块**

1. `book.h` / `book.c` 新增 `removeBook`：按编号或书名匹配节点，单次遍历摘链并 `free` 释放内存（含删除头节点的情况），返回 1/0；
2. `main.c` 管理员功能菜单新增"2.删除图书信息"：输入编号或书名，成功提示"图书删除成功"，未找到提示"未找到编号或名称为 xxx 的图书"，空输入取消。

验证：`gcc -Wall -Wextra` 编译零警告；按编号删除 / 按书名删除 / 未找到 / 空输入 / 删除后浏览确认五条路径实测通过。

**2026-08-13：开发图书信息录入模块**

1. `book.h` / `book.c` 新增 `genBookId`：解析现有 `B%d` 编号取最大 +1，自动生成下一个编号（如 `B009`）；
2. `main.c` 新增"管理员功能"子菜单（登录成功后进入）与"1.图书信息录入"：录入书名/作者/出版社/出版年份/库存数量，校验通过后 `appendBook` 追加，成功回显新书。

验证：`gcc -Wall -Wextra` 编译零警告；录入成功 / 编号递增（B009→B010）/ 非法年份 / 负数库存 / 空书名取消五条路径实测通过。
...
1. 新增 `countBorrowedByUser` 与 `traverseBorrowedByUser`：按借阅人姓名精确匹配、过滤未归还记录，按 `bookId` 查回图书节点后回调输出（图书 + 借阅日期）；
2. 借阅子菜单新增"3.借阅查询"：输入借阅人姓名，显示其全部在借图书的完整信息与借阅日期。

验证：`gcc -Wall -Wextra` 编译零警告；有记录 / 归还后无记录 / 查无此人三条路径实测通过。
